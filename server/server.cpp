...
class Server {
   public:
    Server(boost::asio::io_context& io, unsigned short port)
        : acceptor_(io, tcp::endpoint(tcp::v4(), port)), userStore_("data/users.json") {
   ...
   private:
    void accept_next() {
        acceptor_.async_accept([this](boost::system::error_code ec, tcp::socket socket) {
            if (!ec)
                std::make_shared<Session>(std::move(socket), *this, roomMgr_, userStore_)->start();
            accept_next();
        });
    }
...
void Session::broadcastToRoom(const std::string& roomId, const std::string& data,
                              Session* exclude) {
...
void Session::handleDisconnect() {
    ...
    if (!current_room.empty()) {
        ...
        broadcastToRoom(current_room, json({{"type", "user_left"}, {"username", username}}).dump(),
                        nullptr);
        current_room.clear();
    }
}
...
void Session::handle(const json& msg) {
    ...
    if (type == "login") {
        ...
        if (ok) {
            ...
            send(json({{"type", "login_failed"}, {"reason", "invalid username or password"}})
                     .dump());
        }
        return;
    }
    ...
    if (type == "leave_room") {
        if (!current_room.empty()) {
            ...
            broadcastToRoom(current_room,
                            json({{"type", "user_left"}, {"username", username}}).dump(), this);
            current_room.clear();
        }
        return;
    }
